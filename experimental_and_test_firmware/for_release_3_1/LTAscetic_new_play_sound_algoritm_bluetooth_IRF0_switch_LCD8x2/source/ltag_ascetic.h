...
void get_word_settings(char*, uint8_t*, uint16_t);//Получаем значения настроек с помощью джойстика и ЖКИ
...
void playback_sound(TSOUND_ROLE);//воспроизводим звук через прерывание
...
void test_dt_data(void);//обрабатываем приемный буфер блютус
...
extern uint8_t ir_f0_value [] PROGMEM;
extern volatile trx_packet rx_packet;
...
extern volatile EEMEM TIR_CARRIER_FREQUENCY eeprom_ir_carr_freq;//использемая частота несущей ИК сигнала 
...
extern volatile EEMEM uint16_t sound_7_adress;//адрес в eeprom звука выстрела
extern volatile EEMEM uint16_t sound_7_size;//длина в байтах звука выстрела
extern volatile EEMEM uint16_t sound_8_adress;//адрес в eeprom звука выстрела
extern volatile EEMEM uint16_t sound_8_size;//длина в байтах звука выстрела
extern volatile EEMEM uint16_t sound_9_adress;//адрес в eeprom звука выстрела
...
extern volatile tir_message ir_message;//структура для хранения принятого по ИК сообщения
...
extern volatile tsound_to_play curr_sound;//звук, который воспроизводится в данный момент, или последний воспроизведенный звук 
extern volatile bool status_need_update; //исли true, то нужно обновить данные на ЖКИ
extern volatile uint8_t safe_counter; //счетчик времени спасения

extern volatile uint16_t cyclic_deley_counter; //обратный счетчик длительности между выстрелами при автоматической стрельбе

extern volatile EEMEM uint16_t eeprom_cyclic_rate;//скорострельность, переменная в eeprom

extern volatile EEMEM uint8_t eeprom_safe_duration; //время неуязвимости после ранения (в десятых длях секунды)