...
//Опишем структуру, в которой будем хранить данные для воспроизведения звука
typedef struct sound_to_play {
	TSOUND_ROLE role;	//назначение звука
	uint16_t adress;	//адрес во внешней eeprom
	uint16_t size;		//размер
	uint16_t simples_in_queue;
	bool play_now; //если true - воспроизвести немедленно
} tsound_to_play;
