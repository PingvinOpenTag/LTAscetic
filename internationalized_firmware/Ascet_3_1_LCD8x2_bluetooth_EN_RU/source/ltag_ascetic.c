...
switch(reload_key_event)
	{
		...
		case key_pressing:
		{
			if (reload_state==nothing_to_do)
			{
				...
				if ((clips > 0)&&(curr_sound.simples_in_queue==0))//если обоймы не кончилсь и не воспроизводиться звук выстрела
				{	
...
switch(keyboard_event)
	{
		...
		case key_pressing:
		{
			if(((curr_sound.role==hit_sound)&&(curr_sound.simples_in_queue>0))||(rx_event==RX_COMPLETE))//если звук ранения ещё играет
			{
				keyboard_event=no_key_pressing; //ничего не делаем
				break;
			}
			if(reload_state==nothing_to_do)
			{
...
if (status_need_update) 
	{
		if (!((curr_sound.role==hit_sound)&&(curr_sound.simples_in_queue>0)))//если не играет звук ранения
		{
			display_status();
...
cyclic_deley_counter = (60000/eeprom_read_word(&eeprom_cyclic_rate))*8;//задержка между выстрелами при автоматической стрельбе в тиках таймера
}
...
volatile void get_word_settings(char* text, uint8_t* var_adress, uint16_t max_value){//Получаем значения настроек с помощью джойстика и ЖКИ
	uint16_t result;
...
void get_all_setings(void)
{
	...
	get_word_settings("Выс./мин",&eeprom_cyclic_rate, 800);
	...
	#elif LANGUAGE == EN
	get_int_settings("PlayerID", &eeprom_player_id, 127); //нажата центральная кнопка
	set_player_id(eeprom_read_byte(&eeprom_player_id));	//Устанавливаем идентификатор игрока
	get_int_settings("TeamID", &eeprom_team_id, 3); //нажата центральная кнопка
	set_team_color(team_id());	//Устанавливаем идентификатор (цвет) команды
	...
	get_ir_protocol();
	reinit_timer2();
	get_int_settings("RndInClp", &eeprom_bullets_in_clip, 90); //нажата центральная кнопка
...
void game_over(void){//игра закончена
			life = 0;
			life_in_percent=0;
			WOUND_LED_ON;
			display_life(life);//отобразим уровень жизни на диодах
